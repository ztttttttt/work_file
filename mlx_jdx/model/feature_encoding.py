    ...
    def fit(self, X, y=None):
        ...
        for col in X.columns:
            if len(set(X[col].map(lambda x: type(x))) & set([list, dict, tuple])) != 0:
                ...
                abandoned_cols.append(col)
            elif X[col].dtype not in ['int', 'float'] and X[col].unique().shape[0] > self.max_num:
                abandoned_cols.append(col)

        X_ = X.drop(abandoned_cols, axis=1)


        self.cat_cols = X_.select_dtypes(exclude=['int', 'float']).columns
        self.num_cols = X_.select_dtypes(include=['int', 'float']).columns
